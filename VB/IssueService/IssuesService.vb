	...
	Public Module IssuesService
		...
		Private Function SortIssues(ByVal sortOrder As IssueSortOrder, ByVal issues As IEnumerable(Of IssueData)) As IEnumerable(Of IssueData)
			...
			Case IssueSortOrder.Default
				Return issues.Where(Function(x) x.Created > DateTime.Today.AddDays(-2) AndAlso x.Votes > 70).OrderByDescending(Function(x) x.Created)
			Case IssueSortOrder.Hot
				Return issues.Where(Function(x) x.Created > DateTime.Today.AddDays(-2) AndAlso x.Votes > 70).OrderByDescending(Function(x) x.Votes).ThenByDescending(Function(x) x.Created)
			Case IssueSortOrder.Week
				Return issues.Where(Function(x) x.Created > DateTime.Today.AddDays(-7) AndAlso x.Votes > 90).OrderByDescending(Function(x) x.Votes).ThenByDescending(Function(x) x.Created)
